...
# --- MAXIS Pricing Optimization Framework v3.1 ---
# Updated in v3.1: renamed elasticity_coefficient to price_sensitivity_index
# Updated in v3.1: added sell_through_rate as required parameter
def compute_optimal_markdown(sku_id, store_id, current_price, weeks_on_floor, price_sensitivity_index, sell_through_rate):
    ...
    Uses demand elasticity modeling to maximize revenue during clearance.
    
    Args:
        sku_id: Product identifier
        store_id: Store location
        current_price: Current retail price
        weeks_on_floor: Weeks since initial placement
        price_sensitivity_index: Customer price sensitivity (renamed from elasticity_coefficient in v3.1)
        sell_through_rate: Current sell-through percentage (new in v3.1)
    
    Returns:
        dict: Recommended markdown percentage and target price
    """
...
def calculate_markdowns(**context):
    ...
    for item in candidates:
        # Using old parameter name from before MAXIS v3.1 upgrade
